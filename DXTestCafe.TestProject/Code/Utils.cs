...
namespace DXTestCafe.TestProject.Code
{
	...
	{
		const string SEOSafeReplace = "[^A-Za-z0-9\\s-_\\+]";
		/// <summary>
		/// regexSEOSave
		/// </summary>
		static Regex SEOSafe = new Regex(SEOSafeReplace,
												RegexOptions.IgnoreCase
												| RegexOptions.CultureInvariant
												| RegexOptions.IgnorePatternWhitespace
												| RegexOptions.Compiled
												);
		static Regex SEOReplaceSpaces = new Regex("[\\s]",
												RegexOptions.CultureInvariant
												| RegexOptions.Compiled
												);
		/// <summary>
		...
		/// </summary>
		static Regex SEOReplacePlusses = new Regex("[\\+]",
												RegexOptions.CultureInvariant
												| RegexOptions.Compiled
												);
		...
		public static string MakeSEOSafe(this string value, string spaceReplacement = "-")
		{
			string str = RemoveDiacritics(value);
			str = SEOSafe.Replace(str, string.Empty);
			str = SEOReplaceSpaces.Replace(str, spaceReplacement);
			str = SEOReplacePlusses.Replace(str, spaceReplacement);
			return str;
